package com.missouri_of_god.filecensus;
...
public class FileCensusMain extends JFrame {
    ...
    private FileCensusMain() {
        JTextField tf = new JTextField("", 20);
        JButton b = new JButton("开始普查");
        JButton b2 = new JButton("选择文件（夹）");
        ...
        Checkbox c38 = new Checkbox("所有（只对文件数有效）");

        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setVisible(true);
        setLocation(1680 / 2 - 500 / 2, 946 / 2 - 280 / 2);
        setTitle("File Census 文件普查 -- by Missouri_of_god");
        ...
        c37.addItemListener(e -> hasExtensions[36] = e.getStateChange() == ItemEvent.SELECTED);

        c38.addItemListener(e -> all = e.getStateChange() == ItemEvent.SELECTED);
        ...
        b2.addActionListener(e -> {
            ...
            if (path == null) tf.setText(tf.getText());
            else tf.setText(path.getAbsolutePath());
        });
        ...
        add(c37); add(c38); add(tf); add(b); add(b2);
    ...
    private void paintResult(Graphics g) {
        ...
        g.setColor(Color.black);
        g.setFont(new Font("微软雅黑", Font.BOLD, 25));
        g.drawString("行数：   " + lines, 150, 220);
        g.drawString("字符数：" + chars, 150, 245);
        g.drawString("文件数：" + files, 150, 270);
    }
    ...
    private static void readFromDirectory(File directory) {
        if (directory.listFiles() != null) for (File file : directory.listFiles()) {
